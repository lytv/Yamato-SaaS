...
/**
 * Validate Plans export data
 * @param plans - Plans array to validate
 * @returns Validation result
 */
export function validatePlansExportData(plans: readonly Plan[]): {
  isValid: boolean;
  error?: string;
} {
  if (!Array.isArray(plans)) {
    return { isValid: false, error: 'Plans must be an array' };
  }

  if (plans.length === 0) {
    return { isValid: false, error: 'No plans to export' };
  }

  if (plans.length > 5000) {
    return { isValid: false, error: 'Too many plans to export (maximum 5000)' };
  }

  // Check if plans have required fields
  const invalidPlans = plans.filter(
    plan => !plan.planCode || !plan.planName || !plan.planYear || !plan.planMonth,
  );

  if (invalidPlans.length > 0) {
    return {
      isValid: false,
      error: `${invalidPlans.length} plan(s) missing required fields`,
    };
  }
...
/**
 * Generate Excel workbook from plans array
 * @param plans - Array of plans to export
 * @returns Buffer containing Excel file data
 */
export function generatePlansExcel(plans: readonly Plan[]): Buffer {
  // Create a new workbook
  const workbook = XLSX.utils.book_new();

  // Transform plans data for Excel export
  const excelData = plans.map(plan => ({
    'Plan Code': plan.planCode,
    'Plan Name': plan.planName,
    'Plan Year': plan.planYear,
    'Plan Month': plan.planMonth,
    'Total Target Quantity': plan.totalTargetQuantity || '',
    'Total Actual Quantity': plan.totalActualQuantity || '',
    'Status': plan.status || '',
    'Plan Start Date': plan.planStartDate ? formatDateForExcel(plan.planStartDate) : '',
    'Plan End Date': plan.planEndDate ? formatDateForExcel(plan.planEndDate) : '',
    'Approved By': plan.approvedBy || '',
    'Approved At': plan.approvedAt ? formatDateForExcel(plan.approvedAt) : '',
    'Note': plan.note || '',
    'Created Date': formatDateForExcel(plan.createdAt),
    'Updated Date': formatDateForExcel(plan.updatedAt),
  }));
  ...
  const columnWidths = [
    ...
    { wch: 10 }, // Plan Year
    { wch: 10 }, // Plan Month
    { wch: 15 }, // Total Target Quantity
    { wch: 15 }, // Total Actual Quantity
    { wch: 12 }, // Status
  ...
  const metadataSheet = XLSX.utils.json_to_sheet([
    { Field: 'Export Date', Value: new Date().toISOString() },
    { Field: 'Total Plans', Value: plans.length },
    { Field: 'Generated By', Value: 'Yamato SaaS Plan Management' },
  ]);
