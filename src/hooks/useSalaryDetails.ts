...
import type { SalaryDetailsParams, SalaryDetailsResponse } from '@/types/salaryDetails';
...
async function fetchSalaryDetails(params: SalaryDetailsParams): Promise<SalaryDetailsResponse> {
  ...
  if (params.userIds) {
    searchParams.set('userIds', params.userIds);
  }
  ...
  if (params.showAll) {
    searchParams.set('showAll', params.showAll.toString());
  }
