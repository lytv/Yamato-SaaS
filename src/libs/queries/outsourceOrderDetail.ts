...
export async function getOutsourceOrderDetailById(
  ...
  if (includeRelations) {
    ...
    // Get planned quantity from plan_detail (aggregate by planId and productCode)
    const plannedQuantityResult = await db
      .select({ sum: sum(planDetailSchema.plannedQuantity) })
      .from(planDetailSchema)
      .where(and(
        eq(planDetailSchema.planId, row.outsourceOrderDetail.planId),
        eq(planDetailSchema.productCode, row.outsourceOrderDetail.productCode),
    ...
    return {
      ...
      // Add planned quantity from plan_detail (aggregated)
      plannedQuantity: Number(plannedQuantityResult[0]?.sum) || 0,
