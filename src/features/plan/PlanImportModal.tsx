/**
 * PlanImportModal Component
 * Single-step modal following PlanModal pattern
 * Handles Excel file upload and plan import
 */
...
export function PlanImportModal({ isOpen, onClose, onSuccess }: PlanImportModalProps) {
  const [selectedFile, setSelectedFile] = useState<File | null>(null);
  const { importPlans, isImporting, importError, importResult, clearError, clearResult } = usePlanImport();
  ...
  const handleImport = async () => {
    ...
    try {
      const result = await importPlans(selectedFile);
      onSuccess(result);
