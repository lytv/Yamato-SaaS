/**
 * WorkTableImportModal Component
 * Handles Excel file upload and work table import from ymt_plan format
 */

import React, { useState } from 'react';

import { useWorkTableImport } from '@/hooks/useWorkTableImport';
import type { ImportResult } from '@/types/import';

type WorkTableImportModalProps = {
  isOpen: boolean;
  onClose: () => void;
  onSuccess: (result: ImportResult) => void;
};

export function WorkTableImportModal({ isOpen, onClose, onSuccess }: WorkTableImportModalProps) {
  const [selectedFile, setSelectedFile] = useState<File | null>(null);
  const { importWorkTables, isImporting, importError, importResult, clearError, clearResult } = useWorkTableImport();
  ...
  const handleImport = async () => {
    ...
    try {
      const result = await importWorkTables(selectedFile);
      onSuccess(result);
      ...
      <div className="max-h-[90vh] w-full max-w-2xl overflow-auto rounded-lg bg-white shadow-xl">
        ...
        <div className="p-6">
          <h2 id="import-modal-title" className="mb-4 text-xl font-semibold">
            Import Work Tables from YMT Plan
          </h2>
          ...
          <div className="flex justify-end space-x-3">
            ...
            {!importResult && (
              ...
              >
                {isImporting ? 'Importing...' : 'Import Work Tables'}
              </button>
            )}
          </div>
        </div>
