/**
 * EmployeeSalaryEntryImportModal Component
 * Single-step modal following EmployeeSalaryEntryModal pattern
 * Handles Excel file upload and employeeSalaryEntry import
 */

import React, { useState } from 'react';

import { useEmployeeSalaryEntryImport } from '@/hooks/useEmployeeSalaryEntryImport';
import type { ImportResult } from '@/types/import';

type EmployeeSalaryEntryImportModalProps = {
  isOpen: boolean;
  onClose: () => void;
  onSuccess: (result: ImportResult) => void;
};

export function EmployeeSalaryEntryImportModal({ isOpen, onClose, onSuccess }: EmployeeSalaryEntryImportModalProps) {
  const [selectedFile, setSelectedFile] = useState<File | null>(null);
  const { importEmployeeSalaryEntrys, isImporting, importError, importResult, clearError, clearResult } = useEmployeeSalaryEntryImport();
  ...
  const handleImport = async () => {
    ...
    try {
      const result = await importEmployeeSalaryEntrys(selectedFile);
      onSuccess(result);
      ...
      <div className="max-h-[90vh] w-full max-w-2xl overflow-auto rounded-lg bg-white shadow-xl">
        ...
        <div className="p-6">
          <h2 id="import-modal-title" className="mb-4 text-xl font-semibold">
            Import EmployeeSalaryEntrys from Excel
          </h2>
          ...
          <div className="flex justify-end space-x-3">
            ...
            {!importResult && (
              ...
              >
                {isImporting ? 'Importing...' : 'Import EmployeeSalaryEntrys'}
              </button>
            )}
          </div>
        </div>
