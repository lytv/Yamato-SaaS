...
type OutsourceOrderDetailSummaryProps = {
  ...
  return (
    <div className={`space-y-6 ${className}`}>
      ...
      <div className="rounded-lg border bg-white p-6 shadow-sm">
        <h3 className="mb-4 text-lg font-semibold">Summary Totals</h3>
      ...
      <div className="grid grid-cols-1 gap-6 md:grid-cols-2">
        {/* Completion Progress */}
        <div className="rounded-lg border bg-white p-6 shadow-sm">
          <h4 className="text-md mb-3 font-semibold">Completion Progress</h4>
          <div className="space-y-3">
            ...
            <div className="flex justify-between text-xs text-gray-500">
              <span>
                {summary.totalCompletedQuantity}
                {' '}
                completed
              </span>
              <span>
                {summary.totalOrderedQuantity - summary.totalCompletedQuantity}
                {' '}
                remaining
              </span>
            </div>
        ...
        {/* Status Breakdown */}
        <div className="rounded-lg border bg-white p-6 shadow-sm">
          <h4 className="text-md mb-3 font-semibold">Status Breakdown</h4>
      ...
      <div className="grid grid-cols-1 gap-6 md:grid-cols-2">
        {/* Production Steps */}
        <div className="rounded-lg border bg-white p-6 shadow-sm">
          <h4 className="text-md mb-3 font-semibold">By Production Step</h4>
          <div className="max-h-40 space-y-2 overflow-y-auto">
            {Object.entries(summary.stepCounts)
              .sort(([,a], [,b]) => b - a)
              .map(([step, count]) => (
                <div key={step} className="flex items-center justify-between">
                  <span className="mr-2 flex-1 truncate text-sm text-gray-700" title={step}>
                    {step}
                  </span>
                  <span className="text-sm font-medium">{count}</span>
                </div>
        ...
        {/* Products */}
        <div className="rounded-lg border bg-white p-6 shadow-sm">
          <h4 className="text-md mb-3 font-semibold">By Product</h4>
          <div className="max-h-40 space-y-2 overflow-y-auto">
            {Object.entries(summary.productCounts)
              .sort(([,a], [,b]) => b - a)
              .map(([product, count]) => (
                <div key={product} className="flex items-center justify-between">
                  <span className="mr-2 flex-1 truncate text-sm text-gray-700" title={product}>
                    {product}
                  </span>
                  <span className="text-sm font-medium">{count}</span>
                </div>
