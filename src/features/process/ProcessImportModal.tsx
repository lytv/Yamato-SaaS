/**
 * ProcessImportModal Component
 * Single-step modal following ProcessModal pattern
 * Handles Excel file upload and process import
 */
...
type ProcessImportModalProps = {
  isOpen: boolean;
  onClose: () => void;
  onSuccess: (result: ImportResult) => void;
};

export function ProcessImportModal({ isOpen, onClose, onSuccess }: ProcessImportModalProps) {
  const [selectedFile, setSelectedFile] = useState<File | null>(null);
  const { importProcesses, isImporting, importError, importResult, clearError, clearResult } = useProcessImport();
  ...
  const handleImport = async () => {
    ...
    try {
      const result = await importProcesses(selectedFile);
      onSuccess(result);
        ...
        <div className="p-6">
          ...
          <div className="flex justify-end space-x-3">
            ...
            {!importResult && (
              ...
              >
                {isImporting ? 'Importing...' : 'Import Processs'}
              </button>
            )}
          </div>
        </div>
