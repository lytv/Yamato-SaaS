/**
 * UserSyncImportModal Component
 * Single-step modal following UserSyncModal pattern
 * Handles Excel file upload and user_sync import
 */

import React, { useState } from 'react';

import { useUserSyncImport } from '@/hooks/useUserSyncImport';
import type { ImportResult } from '@/types/import';

type UserSyncImportModalProps = {
  isOpen: boolean;
  onClose: () => void;
  onSuccess: (result: ImportResult) => void;
};

export function UserSyncImportModal({ isOpen, onClose, onSuccess }: UserSyncImportModalProps) {
  const [selectedFile, setSelectedFile] = useState<File | null>(null);
  const { importUserSyncs, isImporting, importError, importResult, clearError, clearResult } = useUserSyncImport();
  ...
  const handleImport = async () => {
    ...
    try {
      const result = await importUserSyncs(selectedFile);
      onSuccess(result);
        ...
        <div className="p-6">
          ...
          <div className="flex justify-end space-x-3">
            ...
            {!importResult && (
              ...
              >
                {isImporting ? 'Importing...' : 'Import UserSyncs'}
              </button>
            )}
          </div>
        </div>
