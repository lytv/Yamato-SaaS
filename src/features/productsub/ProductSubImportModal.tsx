/**
 * ProductSubImportModal Component
 * Single-step modal following ProductSubModal pattern
 * Handles Excel file upload and productsub import
 */

import React, { useState } from 'react';

import { useProductSubImport } from '@/hooks/useProductSubImport';
import type { ImportResult } from '@/types/import';

type ProductSubImportModalProps = {
  isOpen: boolean;
  onClose: () => void;
  onSuccess: (result: ImportResult) => void;
};

export function ProductSubImportModal({ isOpen, onClose, onSuccess }: ProductSubImportModalProps) {
  const [selectedFile, setSelectedFile] = useState<File | null>(null);
  const { importProductSubs, isImporting, importError, importResult, clearError, clearResult } = useProductSubImport();
  ...
  const handleImport = async () => {
    ...
    try {
      const result = await importProductSubs(selectedFile);
      onSuccess(result);
      ...
      <div className="max-h-[90vh] w-full max-w-2xl overflow-auto rounded-lg bg-white shadow-xl">
        ...
        <div className="p-6">
          <h2 id="import-modal-title" className="mb-4 text-xl font-semibold">
            Import Product Subs from YMT Plan
          </h2>
          ...
          <div className="flex justify-end space-x-3">
            ...
            {!importResult && (
              ...
              >
                {isImporting ? 'Importing...' : 'Import Product Subs'}
              </button>
            )}
          </div>
        </div>
