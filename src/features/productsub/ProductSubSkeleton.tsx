/**
 * ProductSubSkeleton Component
 * Following TDD Workflow Standards - Green Phase
 * Provides loading skeleton for productsub list and forms
 */

import React from 'react';

type ProductSubSkeletonProps = {
  rows?: number;
  className?: string;
  variant?: 'desktop' | 'mobile';
};

export function ProductSubSkeleton({
  rows = 3,
  className = '',
  variant = 'desktop',
}: ProductSubSkeletonProps): JSX.Element {
  ...
  if (variant === 'mobile') {
    return (
      <div
        ...
        role="status"
        aria-label="Loading productsubs"
      >
  ...
  return (
    <div
      data-testid="productsub-skeleton"
      className={`${className}`}
      role="status"
      aria-label="Loading productsubs"
    >
